...
def build():
        ...
        weeks.append(
            {
                ...
                "closed": sum(
                    1 for i in issues if i["_done"] and start < i["_done"] <= end
                ),
            }
        )
    ...
    if open_issues:
        avg_age = round(
            sum((now - i["_c"]).days for i in open_issues) / len(open_issues), 1
        )
    ...
    closed_today, closed_7d, closed_30d = (
        since("_done", 1),
        since("_done", 7),
        since("_done", 30),
    )
    ...
    repl = {
        ...
        "__CANCELED__": str(canceled),
        "__IN_PROGRESS__": str(in_progress),
        "__BACKLOG__": str(backlog),
        "__COMPLETION_PCT__": str(dashboard["completion_pct"]),
        "__BURN__": str(burn_rate),
        "__WTC__": wtc_str,
        "__AVG_AGE__": str(avg_age),
        "__OLDEST__": oldest_str,
        "__CREATED_TODAY__": str(created_today),
        "__CREATED_7D__": str(created_7d),
        "__CREATED_30D__": str(created_30d),
        "__CLOSED_TODAY__": str(closed_today),
        "__CLOSED_7D__": str(closed_7d),
